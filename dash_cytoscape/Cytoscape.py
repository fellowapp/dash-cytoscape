...
- operation (dict; optional):
    [Fellow] An invocation mechanism for customized, pre-defined
    operations on the graph.      1. Expected dictionary attributes:
    - `command` (string): Name of the operation     - `timestamp`
    (string): Set a time later than the previous timestamp.     The
    component will not re-apply an operation with a stale timestamp.
    - `args` (dictionary): Arguments required by the operation.
    2. Supported operations:     - `locateNode`: Position and zoom the
    graph viewport over the element     specified by the `selector`
    (and, therefore, allows for searching the     graph).         -
    Argument:             - `selector`: The node to locate in
    Cytoscape selector syntax      3. Limitations:     - The operation
    happens before the Cytoscape element renders.
